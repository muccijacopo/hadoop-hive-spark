...
CREATE TABLE IF NOT EXISTS sectors_best_stock_by_volumes_year AS
    ...
    FROM (
        SELECT *, ROW_NUMBER() OVER (PARTITION BY t2.sector, t2.year_ ORDER BY t2.sector, t2.year_, t2.total_volume DESC) rn
        FROM (
...
CREATE TABLE IF NOT EXISTS sectors_total_price_var_year AS
